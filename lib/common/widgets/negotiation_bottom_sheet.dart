...
class _NegotiationBottomSheetContentState
  ...
  void _fetchNegotiationId() {
    ...
    final controller =
        Provider.of<SpNegotiationsByIdEmailCtrl>(context, listen: false);
  ...
  Future<void> _sendMessage(SpNegotiationRoundCtrl roundCtrl) async {
    ...
    if (priceText.isNotEmpty) {
      offeredPrice = double.tryParse(priceText);
    ...
    if (message.isEmpty && offeredPrice == null) return;
    ...
    try {
      ...
      // Provider's Consumer automatically handles the data refresh and list update

      if (!mounted) return;

      // 1. Create and show the Flushbar
      final flushbar = Flushbar(
        message: 'Offer sent! Closing sheet...',
        // Increased duration for visibility before closing
      ...
      // AWAIT the flushbar to ensure it has time to appear and dismiss
      await flushbar.show(context);
      
      // 2. Close the sheet only after the flushbar has been shown
      if (mounted) {
        Navigator.pop(context, true); 
      }

    } catch (e) {
      if (kDebugMode) print('❌ Failed to send negotiation round: $e');
      showTopNotification(context, 'Failed to send negotiation round.', isError: true);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<SpNegotiationsByIdEmailCtrl>(
      builder: (context, controller, _) {
        ...
        if (negotiations.isNotEmpty && negotiations.first.status == 'accepted') {
          _isAccepted = true;
        }
        ...
        if (_displayedRounds.isEmpty) {
          return Column(children: [_buildPlaceholder(), _buildInputAndAcceptArea()]);
        }
  ...
  Widget _buildAnimatedRound(
      NegotiationRound round, Animation<double> animation) {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    round.createdAt != null ? DateFormat('HH:mm').format(round.createdAt!) : 'N/A',
                    style: const TextStyle(
  ...
  Widget _buildInputAndAcceptArea() {
    return Consumer2<SpNegotiationRoundCtrl, ProviderAcceptNegotiationCtrl>(
      builder: (context, roundCtrl, acceptCtrl, _) {
        ...
        final lastSenderIsClient = _displayedRounds.isNotEmpty && 
        ...
        final bool shouldShowControls = !isNegotiationAccepted && 
                                        (isFirstMessage || lastSenderIsClient);
        
        // The Accept button requires controls to be shown AND a client offer with a price
        final bool shouldShowAccept = shouldShowControls &&
        ...
        return Padding(
          padding: EdgeInsets.only(
            left: 16, right: 16, bottom: bottomInset + 8, top: 8,
          ),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: acceptCtrl.isLoading
                          ...
                          : () async {
                              ...
                              try {
                                ...
                                // Revert state if the acceptance fails
                                setState(() => _isAccepted = false); 
                                if (kDebugMode) print('❌ Accept failed: $e');
                      ...
                      child: acceptCtrl.isLoading
                          ...
                          : Text('Accept R${_displayedRounds.last.offeredPrice?.toStringAsFixed(2) ?? '?' }', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ),
                  ),
                ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: IconButton(
                      ...
                      onPressed: roundCtrl.isLoading ? null : () => _sendMessage(roundCtrl),
                    ),
                  ),
                ],
              ),
