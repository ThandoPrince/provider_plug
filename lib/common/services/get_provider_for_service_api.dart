    ...
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> rawList =
            decodedMap['data'] is List ? decodedMap['data'] : [];

        final services = rawList
            .map(
              (item) => ProviderServiceModel.fromJson(
      ...
      if (kDebugMode) {
        print(
          '❌ PROVIDER SERVICES QUERY REJECTED [${response.statusCode}]: ${response.body}',
        );
      }
      ...
      return {
        ...
        'statusCode': 500,
        'message':
            'Network or parsing error while loading provider profile details.',
      };
    }
  }
}