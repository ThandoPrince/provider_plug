...
class BookingDetailContent extends StatelessWidget {
  ...
  Future<void> showNegotiationBottomSheet(
      BuildContext context, int orderId, String providerEmail) async {
    ...
    if (result == true) {
      Provider.of<BookingByOrderIDController>(context, listen: false)
      ...
      child: Column(
        ...
        children: [
          ...
          DetailCard(
            ...
            children: [
              DetailRow(label: "Name", value: "${booking.client?.firstName ?? ""} ${booking.client?.lastName ?? ""}", color: secondaryText),
              DetailRow(label: "Rating", value: booking.client?.rating ?? "N/A", color: secondaryText),
            ],
          ),
const SizedBox(height: kCardSpacing),
          ...
          DetailCard(
            ...
            primaryTextColor: primaryText,
            children: [
              DetailRow(label: "Required On", value: formatDateTime(booking.requestedDateTime), color: secondaryText),
              DetailRow(label: "Proposed Price", value: "R ${booking.proposalPrice?.toStringAsFixed(2) ?? "0.00"}", color: secondaryText),
              DetailRow(label: "Service Category", value: booking.serviceRequired?.serviceName ?? "N/A", color: secondaryText),
                ...
                child: InkWell(
                  onTap: () {
                    if (booking.orderId != null) {
                      ...
                      showNegotiationBottomSheet(context, booking.orderId!, hardCodedEmail);
                    }
                  },
                  borderRadius: BorderRadius.circular(16),
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.handshake_outlined, color: Colors.blueAccent, size: 22),
                        const SizedBox(width: 12),
                        const Text("VIEW NEGOTIATION", style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.w800, fontSize: 13, letterSpacing: 0.5)),
                        const Spacer(),
                        Icon(Icons.arrow_forward_ios_rounded, color: Colors.blueAccent.withOpacity(0.5), size: 14),
                      ],
                    ),
                  ),
                ),
...
const SizedBox(height: kCardSpacing),
              ...
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.location_on_rounded, size: 18, color: Colors.redAccent),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
                      booking.deliveryAddress?.formattedAddress ?? "No address provided",
                      style: const TextStyle(fontSize: 14, color: secondaryText, height: 1.4, fontWeight: FontWeight.w600),
                    ),
                  ),
                ],
              )
          ...
          // IMAGES
          if (booking.orderPictures != null && booking.orderPictures!.isNotEmpty)
            BookingImagesList(
